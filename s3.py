...
#    expires_seconds = expires * 60 * 60  # convert to seconds
    expires_seconds = expires * 60   # convert to seconds
#    expire_date = datetime.now() + timedelta(hours=expires)
    expire_date = datetime.now() + timedelta(minutes=expires)
    ...
    print '######## GENERATED URL(EXPIRE %s %s seconds) ########' % (expire_date, expires_seconds)
    print url
